...
siteCounties <-  read.csv(paste0(root, "/Indicator_Output/Siting_Counties_MasterList.csv"), 
                          stringsAsFactors = FALSE, colClasses = c("FIPS"="character")) ## GRC: what does this do?
## GRC: GO BACK TO TEST THIS ^
head(siteCounties)
...
mystates = c("AZ", "TX", "FL", "GA", "MI", "NC","PA","WI")

# Create a df that can interact with tidycensus 
siteCounties$county_code = substr(siteCounties$FIPS, 3, 5) #GRC: Is this a useless line of code? 

# use built-in dataset 'fips_codes' to select the fips codes I want
my_counties <- 
  fips_codes %>%
...
# export
write.csv(popBlock, paste0(root, "/Indicator_Output/decennial/population_Block_2010_Decennial_2.csv"), row.names = FALSE)

############# 1. Calculate CVAP Population Share ############# 
# read in CVAP (voting age pop) data # using the updated CVAP for the demographic components of the model
cvap = fread(paste0(root, "/Indicator_Inputs/acs/source/CVAP_2014-2018_ACS_csv_files/Tract.csv"), data.table = FALSE)
...
cvapDens <- lapply(cvapDens, function(x) within(x, cvap_est.Tot <- sum(x$cvap_est))) #GRC added this line because it was having a hard time looping through a single line
cvapDens <- lapply(cvapDens, function(x) within(x, cvapDens.MOE <- moe_prop(x$cvap_est, x$cvap_est.Tot, x$cvap_moe, x$cvapTot.MOE)))
...
cvapDensDF <- cvapDensDF[,c(10, 1, 9, 7,8, 11:12,14:15)] # GRC: Can these stay the same across all states?
...
# EXPORT
write.csv(cvapDensDF, paste0(root, "/Indicator_Inputs/acs/CVAP_ShareOfCVAPPopulation.csv"), row.names = FALSE)
...
# export acs data
write.csv(lepTract, paste0(root, "/Indicator_Inputs/acs/LEP.csv"), row.names = FALSE)
...
# export acs data
write.csv(carTract, paste0(root, "/Indicator_Inputs/acs/NoCarAccess.csv"), row.names = FALSE)
...
# export acs data
write.csv(disabTract, paste0(root, "/Indicator_Inputs/acs/DisabledPop.csv"), row.names = FALSE)
...
# export acs data
write.csv(povTract, paste0(root, "/Indicator_Inputs/acs/PovertyPop.csv"), row.names = FALSE)
...
# export acs data
write.csv(youthTract, paste0(root, "/Indicator_Inputs/acs/YouthPop.csv"), row.names = FALSE)
...
# export acs data
write.csv(raceEthTract, paste0(root, "/Indicator_Inputs/acs/RaceEthPop.csv"), row.names = FALSE)
...
cvapDensDF = read.csv(paste0(root, "/Indicator_Inputs/acs/CVAP_ShareOfCVAPPopulation.csv"), stringsAsFactors = FALSE, colClasses = c("FIPS"="character", "GEOID"="character"))
lepTract = read.csv(paste0(root, "/Indicator_Inputs/acs/LEP.csv"), stringsAsFactors = FALSE, colClasses = c("GEOID"="character"))
carTract = read.csv(paste0(root,"/Indicator_Inputs/acs/NoCarAccess.csv"), stringsAsFactors = FALSE, colClasses = c("GEOID"="character"))
disabTract = read.csv(paste0(root,"/Indicator_Inputs/acs/DisabledPop.csv"), stringsAsFactors = FALSE, colClasses = c("GEOID"="character"))
povTract = read.csv(paste0(root,"/Indicator_Inputs/acs/PovertyPop.csv"), stringsAsFactors = FALSE, colClasses = c("GEOID"="character"))
youthTract = read.csv(paste0(root,"/Indicator_Inputs/acs/YouthPop.csv"), stringsAsFactors = FALSE, colClasses = c("GEOID"="character"))
raceEthTract = read.csv(paste0(root, "/Indicator_Inputs/acs/RaceEthPop.csv"), stringsAsFactors = FALSE, colClasses = c("GEOID"="character"))
...
# convert mod NAs to 0, unless it's a flag column, then convert to 1
acsVarsDFFinal_mod$cvapDens[is.na(acsVarsDFFinal_mod$cvapDens)] <- 0
acsVarsDFFinal_mod$cvapDens_flag[is.na(acsVarsDFFinal_mod$cvapDens_flag)] <- 1

summary(acsVarsDFFinal_mod)

# eliminate commas in name fields
...
acsVarsDFFinal_mod$State = ifelse(acsVarsDFFinal_mod$NAME %like% ", Arizona", "AZ", 
                                  ifelse(acsVarsDFFinal_mod$NAME %like% ", California", "CA",
                                         ifelse(acsVarsDFFinal_mod$NAME %like% ", Florida", "FL",
                                                ifelse(acsVarsDFFinal_mod$NAME %like% ", Georgia", "GA",
                                                       ifelse(acsVarsDFFinal_mod$NAME %like% ", Michigan", "MI",
                                                              ifelse(acsVarsDFFinal_mod$NAME %like% ", North Carolina", "NC",
                                                                     ifelse(acsVarsDFFinal_mod$NAME %like% ", Pennsylvania", "PA",
                                                                            ifelse(acsVarsDFFinal_mod$NAME %like% ", Texas", "TX",
                                                                                   ifelse(acsVarsDFFinal_mod$NAME %like% ", Wisconsin", "WI", "check")))))))))

# web visual
acsVarsDFFinal_viz$State = ifelse(acsVarsDFFinal_viz$NAME %like% ", Arizona", "AZ", 
                                  ifelse(acsVarsDFFinal_viz$NAME %like% ", California", "CA",
                                         ifelse(acsVarsDFFinal_viz$NAME %like% ", Florida", "FL",
                                                ifelse(acsVarsDFFinal_viz$NAME %like% ", Georgia", "GA",
                                                       ifelse(acsVarsDFFinal_viz$NAME %like% ", Michigan", "MI",
                                                              ifelse(acsVarsDFFinal_viz$NAME %like% ", North Carolina", "NC",
                                                                     ifelse(acsVarsDFFinal_viz$NAME %like% ", Pennsylvania", "PA",
                                                                            ifelse(acsVarsDFFinal_viz$NAME %like% ", Texas", "TX",
                                                                                   ifelse(acsVarsDFFinal_viz$NAME %like% ", Wisconsin", "WI", "check")))))))))
...
for (i in seq_along(acsVarsFinal_mod)) {
  ...
  write.csv(acsVarsFinal_mod[[i]], paste0(root, "/Indicator_Output/", filename), row.names = FALSE)
}
