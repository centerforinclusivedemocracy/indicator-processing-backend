...
lodesFiles <- list.files(path = "/Users/lauradaly/Documents/GreenInfo/Contract Work/Siting Tool/data/lehd_lodes/", full.names=TRUE) 
...
for (i in seq_along(wrkBlock)) {
  ...
  write.csv(wrkBlock[[i]], paste0(root, "data/output/", filename), row.names = FALSE)
}
...
for (i in seq_along(wrkTract)) {
  ...
  write.csv(wrkTract[[i]], paste0(root, "data/output/visualize", filename), row.names = FALSE)
}
