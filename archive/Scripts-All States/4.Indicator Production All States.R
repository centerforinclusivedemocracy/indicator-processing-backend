...
indicatorFiles <- list.files(path = "/Users/lauradaly/Documents/GreenInfo/Contract Work/Siting Tool/data/output/", full.names=TRUE) 

# I want to remove the all ca counties df because i don't fully know what workflow you will have set up--but it may be that you want the reverse instead (only all CA counties)
indicatorFiles <- indicatorFiles[!indicatorFiles %like% "_AllCACounties"]
...
pop <- 
  lapply(popfiles, function(i){
    fread(i, header=TRUE, stringsAsFactors = FALSE, 
          colClasses = c("GEOID10"="character", "TRACTCE10"="character", "COUNTYFP10"="character", "STATEFP10"="character"), data.table = FALSE)
  })
...
pop <- lapply(pop, function(x) x %>% 
                dplyr::mutate(FIPS = paste0(STATEFP10, COUNTYFP10),
                       pop2010 = value) %>% 
                dplyr::select(GEOID10, FIPS, pop2010, area_km2, popDensKM2, popPrcCounty)) 
...
elig <- 
  lapply(eligfiles, function(i){
    fread(i, header=TRUE, stringsAsFactors = FALSE, 
          colClasses = c("GEOID"="character", "FIPS"="character"), data.table = FALSE)
  })
head(elig[[3]])

# rename col 
elig <- lapply(elig, function(x) x %>% 
                 select( GEOID,  FIPS, Tot_EligNonReg_prc_FINAL, TotElig_flag) %>% 
                 rename( "ElNonReg_flag" = TotElig_flag))

summary(elig[[2]]) # look for NAs
...
poll <- 
  lapply(pollfiles, function(i){
    fread(i, header=TRUE, stringsAsFactors = FALSE,
          colClasses = c("FIPS"="character"), data.table = FALSE)
  })

# standardize names
pollnames <- c("GEOID10",  "FIPS", "pollShare")
...
# standardize names
vbmnames <- c("GEOID10",  "FIPS", "TotVBM", "AsnVBM", "LatVBM", "YouthVBM")
...
for (i in seq_along(dat_state_export)) {
  ...
  write.csv(dat_state_export[[i]], paste0(root, "data/output/model data/test/", filename), row.names = FALSE)
}
