...
cvap = cvap[cvap$FIPS == site_tx$FIPS & cvap$lntitle =="Total", ]
dim(cvap)
head(cvap)
...
prec <- read_sf(dsn="/Users/lauradaly/Documents/GreenInfo/Contract Work/Siting Tool/Texas Siting Tool/data/voter", layer="Harris_County_Voting_Precincts")
head(prec)


# load census block geometry
blocksTX <- read_sf(dsn="/Users/lauradaly/Documents/GreenInfo/Contract Work/Siting Tool/data/decennial", layer="blocksTX")
...
setwd("/Users/lauradaly/Documents/GreenInfo/Contract Work/Siting Tool/Texas Siting Tool/data/")

# Export reduced shapefiles here:
write_sf(intr, "blocks_prec_conversion_TX.shp")

##### IMPORT BLOCK-PRECINCT CONVERSION HERE #####
# can ignore the above part since the intersect takes a while
library(foreign)
...
# remove NAs
regCVAP <- regCVAP[!is.na(regCVAP$CVAP_EST), ]
summary(regCVAP)
