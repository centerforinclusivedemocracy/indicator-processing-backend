...
# -25 lines for heads/tails to go in .Rmd
# - 10 with summaries
...
census_api_key("13c80897021c5b004d6fe7f6d7a31ef3f3e79a90", install = TRUE, overwrite = TRUE)
readRenviron("~/.Renviron")

## READ IN COUNTIES INCLUDED IN THE SITING TOOL 
...
# prepare block 2010 population data - takes ~2m
...
lepTract <- acs_map(lepVars, "B06007_001")
...
carTract <- acs_map(carVars, "B25044_001")
...
disabTract <- acs_map(disabVars, "B23024_001")
...
povTract <- acs_map(povVars, "B17001_001")
...
youthTract <- acs_map(youthVar, "B01001_001")
...
raceEthTract <- acs_map(raceVarList, "B03002_001")
...
## MODEL DATA
# summarize by indicator by tract -- some variables need to be collapsed/summed, for example the LEP variables
acsVarsSum_mod <- tract_sum('flag_zero')  # THERE CAN BE NO NAs, REPLACE WITH ZERO
...
acsVarsSum_viz <-  tract_sum('flag_na')
...
# join in total population data. This will be used when the block population isn't available(new tracts?)
...
# model data
acsVarsDFFinal_mod$State <- ifelse_chain(acsVarsDFFinal_mod)

# web visual
acsVarsDFFinal_mod$State <- ifelse_chain(acsVarsDFFinal_viz)
